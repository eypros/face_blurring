...
def main():
    ...
    cv2.putText(frame, "Press r to select ROI(s) or press s to skip", (50, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.75,
                (0, 0, 255), 2)
    cv2.putText(frame, "or press 1, t, h for 1,10,100 frames skipped", (20, 100),
    ...
    while True:
        ...
        if bboxes:
            ...
            if skip > 0:
                if verbose:
                    print('Frame #{}. Skipping for another {} frames...'.format(int(video.get(cv2.CAP_PROP_POS_FRAMES)), skip))
                skip -= 1
                cv2.imshow("Tracking", frame_blurred)
                continue
            elif skip == -1:
