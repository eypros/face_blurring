...
video_input = '/home/gorfanidis/Datasets/web-images/blur_faces/Jet Blue Flight 292 - SNL_2s.mp4'
SCORE_THRESH = 0.35
ENLARGE_FACTOR = 0.1
__version__ = '0.1'
...
def get_arguments():
    ...
    parser.add_argument("--score-threshold", type=float, default=ENLARGE_FACTOR,
...
def get_bboxes(boxes_det, scores_det, classes_det, image, score_thres=0.5, enlarge_factor=0.1):
    ...
    # # check for extreme values i.e. too distorted bboxes or with too little space
    # # this removes bboxes with a dimension less than 10 pixels
    # n = np.where(boxes_det[:, 2] - boxes_det[:, 0] > 10) and np.where(boxes_det[:, 3] - boxes_det[:, 1] > 10)
    # boxes_det = boxes_det[n]
    # classes_det = classes_det[n]
    # scores_det = scores_det[n]
    # # this removes too narrow or long bboxes (one dimension is at least extreme_factor[0] bigger than the other
    # n = np.where(np.max((boxes_det[:, 2] - boxes_det[:, 0], boxes_det[:, 3] - boxes_det[:, 1]),
    #                     axis=0) / np.min((boxes_det[:, 2] - boxes_det[:, 0], boxes_det[:, 3] -
    #                                       boxes_det[:, 1]), axis=0) < extreme_factor[0])
    # boxes_det = boxes_det[n]
    # classes_det = classes_det[n]
    # scores_det = scores_det[n]
    # # this removes all bboxes with dimensions greater than extreme_factor[1] of the frame dimensions
    # n = np.where(w / (boxes_det[:, 2] - boxes_det[:, 0]) < extreme_factor[1]) or \
    #     np.where(h / (boxes_det[:, 3] - boxes_det[:, 1]) < extreme_factor[1])
    # boxes_det = boxes_det[n]
    # classes_det = classes_det[n]
    # scores_det = scores_det[n]
    #
    ...
    # # overlap = False
    # for i in range(boxes_det.shape[0]):
    #     if i not in removable_bboxes:
    #         for j in range(i + 1, boxes_det.shape[0]):
    #             if j not in removable_bboxes:
    #                 overlaps = check_overlap_percentage(boxes_det[i, :], boxes_det[j, :])
    #                 if (overlaps[0] > overlap_threshold and overlaps[1] > overlap_threshold) or \
    #                         (classes_det[i] == classes_det[j] and (overlaps[0] > overlap_threshold or
    #                                                        overlaps[1] > overlap_threshold)):
    #                     removable_bboxes.append(j)
    # if removable_bboxes:
    #     accepted_values = [x for x in range(boxes_det.shape[0]) if x not in removable_bboxes]
    #     boxes_det = boxes_det[accepted_values, :]
    #     scores_det = scores_det[accepted_values]
    #     classes_det = classes_det[accepted_values]
